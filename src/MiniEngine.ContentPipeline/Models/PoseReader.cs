...
namespace MiniEngine.ContentPipeline.Models
{
    public class PoseReader : ContentTypeReader<Pose>
    {
        protected override Pose Read(ContentReader input, Pose existingInstance)
        {
            ...
            var material = ReadMaterial(input, content);
            var bounds = ReadBoundingSphere(input);
            var vertexBuffer = ReadVertexBuffer(input, graphics);
            var indexBuffer = ReadIndexBuffer(input, graphics);
        ...
        private static Material ReadMaterial(ContentReader input, ContentManager content)
        {
        ...
        private static IndexBuffer ReadIndexBuffer(ContentReader input, GraphicsDeviceManager graphics)
        {
            ...
            var indexBuffer = new IndexBuffer(graphics.GraphicsDevice,
