...
namespace MiniEngine.Pipeline.Shadows.Systems
{
    public sealed class ShadowMapSystem : ISystem
    {
        ...
        private readonly IComponentContainer<CascadedShadowMap> CascadedShadowMaps;
        ...
        public void RenderShadowMaps()
        {
            ...
            for (var i = 0; i < this.ShadowMaps.Count; i++)
            {
                ...
                this.RenderShadowMap(shadowMap);
            }

            for (var i = 0; i < this.CascadedShadowMaps.Count; i++)
            {
                var cascadedShadowMap = this.CascadedShadowMaps[i];
                for (var cascade = 0; cascade < cascadedShadowMap.Cascades; cascade++)
                {
                    this.RenderShadowMap(cascadedShadowMap.ShadowMaps[cascade]);
                }
            }
