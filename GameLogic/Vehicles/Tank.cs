...
namespace MiniEngine.GameLogic.Vehicles
{
    public sealed class Tank
    {
        ...
        private const float TrackLengthModifier = 1.2f;
        private static readonly Vector3 LeftOffset = new Vector3(0, 0, -1);
        private static readonly Vector3 RightOffset = new Vector3(0, 0, 1);
        ...
        public void MoveAndTurn(Vector3 position, float yaw)
        {
            ...
            var oldLeftOf = Vector3.Transform(LeftOffset * unscaledWidth * 0.5f, this.Pose.Matrix);
            var oldRightOf = Vector3.Transform(RightOffset * unscaledWidth * 0.5f, this.Pose.Matrix);

            this.Pose.Move(position);
            this.Pose.Rotate(yaw, this.Pose.Pitch, this.Pose.Roll);

            var newLeftOf = Vector3.Transform(LeftOffset * unscaledWidth * 0.5f, this.Pose.Matrix);
            var newRightOf = Vector3.Transform(RightOffset * unscaledWidth * 0.5f, this.Pose.Matrix);
        ...
        {
            // TODO: forward is +X for tanks, but +Z for world!
            ...
            if (diff <= (MathHelper.PiOver2 + 0.01f))
            {
                return 1.0f;
            }
